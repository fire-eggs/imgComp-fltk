...
void readPhash(char* filename, int sourceId)
{
    ...
    while (fgets(buffer, 2048, fptr))
    {
        ...
        // Lines of the form <hash>|<crc>|<zipfile>|<imagefile>

        char* parts = strtok(buffer, "|");  // hash
        ...
        parts = strtok(NULL, "|"); // zipfile
        if (!parts) // unexpected data
            continue;

        // TODO store the zipfile
        int archiveIndex = addArchive(parts);

        parts = strtok(NULL, "|"); // image file
        if (!parts) // unexpected data
            continue;
